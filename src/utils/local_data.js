import { Mocked_User } from "./User_data";
...
export class mocked_Data {
  ...
  static get_mock_activity_info(id) {
    let _data = USER_ACTIVITY;
    _data = _data.find((user) => user.userId == id);
  

    for (let i = 0; i < _data.sessions.length; i++) {
      _data.sessions[i].day = i + 1;
    }
    for (let j = 0; j < 3; j++) {
      _data.sessions.push({..._data.sessions[j], day:j+8});
    }
    return _data;
  }

  static get_mock_performance_info(id) {
    let _data = USER_PERFORMANCE;
    _data = _data.find((user) => user.userId == id);
    return _data;
  }
  static get_mock_sessions_info(id) {
